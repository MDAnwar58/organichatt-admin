...
export default function PrivacyPolicyForm() {
   ...
   }, [PrivacyPolicyId, des, dispatch]);

   const errors = useSelector((state: any) => state.errors);
   const privacy_policy = useSelector((state: any) => state.privacy_policy);

   useEffect(() => {
      if (privacy_policy.id) {
         setEditorValue(privacy_policy.content);
         setPrivacyPolicyId(privacy_policy.id);
      }
   }, [privacy_policy.id, privacy_policy.content]);

   return (
      <div className="mx-auto w-full pt-0  bg-gray-50 dark:bg-gray-800 shadow-sm border dark:border-gray-900 rounded-md my-5">
         <FormHeader title="Privacy Policy Added" />
         <Form className={"p-5"}>
            <div className="mb-5">
               <TextEditor
